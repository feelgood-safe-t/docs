...
| **P5** | **엔진은 응답을 해석하거나 저장하지 않는다.** 요청의 원본 응답을 검증하고 Snapshot에 구조화하며, 성향 해석·추천이 필요하면 외부 서비스가 수행한다 |
...
> 제공한다. 프론트는 `GET /v1/poc/questionnaire` 응답을 사용해 중복 정본을 만들지 않는다.
...
- 나머지 문항: 정확히 1개 선택
- 엔진은 원본 선택을 저장하지 않고 요청마다 검증하며, 성향 등급이나 위험 한도로 변환하지 않는다.

...
> **정정 (2026-09-03)** — 이전 판에서 문항 대부분을 "채점 기준선"으로 표기했으나 **근거가 부족한 확대 해석이었다.**
> 온보딩 설문의 목적은 **시뮬레이션 시나리오 매칭**이다. 근거는 다음과 같다.
>
> - 백엔드 엔진 스펙: *"엔진은 응답을 **성향 값이나 위험 한도로 변환하지 않아야 한다**"*
> - 책임 경계표: 설문 해석과 문항 추천은 **Selection Orchestrator**(= 시나리오 선택) 담당
> - 채점은 **Evaluation Service**가 봉인된 Snapshot의 **행동 기록**을 근거로 수행한다
>
...
### 2.3 채점과 연결될 가능성이 있는 문항 — 2개뿐, 모두 미확정

| 문항 | 관련 배점 항목 | 근거 | 상태 |
|---|---|---|---|
| `assessment-risk` (7) | 「위험 한도 준수」 20 | 스펙 평가 기준이 *"**사전에 밝힌** 위험 수준과 실제 판단의 일치 여부"*로 정의되어 있고, **"사전에 밝힌 위험 수준"의 출처는 이 문항뿐**이다 | 🔴 미확정 — §4 |
| `leverage` (3) | 「레버리지 통제」 20 | 백엔드 스펙: *"PoC Runtime Scenario에는 레버리지·인버스 상품을 넣지 않는다. **관련 개념은 설문이나 선택적 quiz로 확인**할 수 있다"* | 🔴 미확정 — §4 |

**나머지 8개 문항은 채점과 연결되지 않는다.**
...
## 4. 🔴 미해결 — 배점 50점의 측정 근거가 없다

스펙 배점표 100점 중 **세 항목 50점**이 현재 PoC 구조에서 측정 입력을 갖지 못한다.

### 4.1 「레버리지 통제」 20점 — 레버리지 상품이 PoC에 없다

> **PoC Runtime Scenario에는 레버리지·인버스 상품을 넣지 않는다.**
> 관련 개념은 설문이나 선택적 quiz로 확인할 수 있다. — `backend/docs/backend-engine-spec.md`

레버리지 자산이 시나리오에 등장하지 않으면 *"레버리지 자산 선택과 변동성 확대 후 대응"*을 **행동으로 관찰할 수 없다.**
남는 입력은 온보딩 `survey-q-leverage`(자기신고 이해도)와 선택적 퀴즈뿐인데, 이는 **지식 확인이지 행동 통제가 아니다.**

> ⚠️ **이 사실은 서비스 정체성과도 관련된다.** 기획서상 서비스명이 「AI **레버리지** 안전투자 훈련 플랫폼」이고,
> 문제 정의 전체가 2026-08-19 단일종목 레버리지 모의거래 이수 의무화에 걸려 있다.
> PoC에 레버리지 상품이 없다면 **기획서의 서술 범위를 조정하거나, 시나리오에 레버리지를 포함하도록 논의가 필요**하다.

### 4.2 「위험 한도 준수」 20점 — 정성 3단계로는 한도 초과를 계산할 수 없다

`survey-q-assessment-risk`는 **낮음/보통/높음**이고, 엔진은 규범적으로 다음을 생성하지 않는다.

> 주문, 보유량, 비중, 현금, 노출, 손익, MDD, 유효 레버리지, 집중도

즉 *"밝힌 한도를 얼마나 넘었는가"*를 계산할 정량 입력이 없다.

### 4.3 「자산 집중도」 10점 — item당 자산이 하나면 성립하지 않는다

백엔드 v0.4는 **item당 단일 자산**이다. 한 자산만 있는 상태에서 집중도는 정의되지 않는다.

### 4.4 어디서 정하는가

`evaluationPlanRef`가 가리키는 **외부 승인 rubric**이 *"각 평가 항목의 공개 정보·행동·시장 checkpoint와 증거 부재 처리 규칙"*을 정의하도록 되어 있다. **이 rubric은 아직 작성되지 않았고 담당도 정해지지 않았다.**

**제안 (9/6 미팅 안건)**

| 항목 | 재정의 안 |
|---|---|
| 레버리지 통제 20 | 시나리오에 레버리지를 넣지 않는다면 → **「고변동 구간 대응」**으로 재정의하고, 급변 구간에서의 판단 유지·번복 패턴으로 측정. 레버리지 개념 자체는 **기초 개념 퀴즈**에서 확인 |
| 위험 한도 준수 20 | **「위험 인식 일관성」**으로 재정의 — 선언한 변동 수준(low/medium/high)과 실제 판단·근거의 정합 |
| 자산 집중도 10 | 폐지하고 「판단의 일관성」에 흡수, 또는 배점을 「정보 출처 검증」으로 이동 |

> 어느 안이든 **rubric 작성이 선행**되어야 하며, 기능명세서 4번(AI 및 데이터 처리 방식)에 기재할 내용이다.

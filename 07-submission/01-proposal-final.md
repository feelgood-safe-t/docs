...
이름은 청산당하지 않겠다는 뜻이다. 고위험 상품에서 원금을 잃는 상황을 겪지 않도록 미리 훈련시키겠다는 목표를 그대로 담았다.
...
2026년 8월 19일부터 단일종목 레버리지 상품에 처음 투자하려면 모의거래를 이수해야 하도록 정책이 변경되었다. 그러나 이 요건은 5거래일 동안 매일 1시간이라는 교육 시간 이수 조건일 뿐, 이수자가 위험을 이해했는지는 측정하지 않는다. 이수증은 발급되지만 이수자가 제대로 이해했는지와 같은 학습 이수 수준은 검증되지 않는다.

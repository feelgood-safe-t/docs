...
| 원칙 | 내용 |
|---|---|
| AI로 직접 시장을 만들지 않는다. | 준비된 시나리오의 가격과 거래량은 실제 기록을 바탕으로 하며, AI는 성향 분석, 시나리오 선택, 콘텐츠 각색, 판단 해석만 맡는다. 시나리오 선택은 학습된 머신러닝 분류 모델이, 나머지는 생성형 모델이 수행한다. |
...
![가명 자산과 60배속 캔들, 남은 시간, 확신도와 판단 근거 입력이 한 화면에 있다](figures/shot-1-assessment.png)

자산명은 가명이고 가격은 시작 시점 기준으로 정규화되어 있다. 어떤 시장의 언제였는지 알 수 없는 상태에서 판단하고, 그 판단에 확신도와 근거를 함께 남긴다.
...
수익률이나 방향 적중률은 어디에도 나타나지 않는다. 항목마다 점수와 함께, 응답 시점에 무엇을 볼 수 있었고 실제로 무엇을 열어 봤는지를 짚는 근거가 붙는다.
...
| 시나리오 선택 (랜덤 포레스트) | 성향 프로파일, 검수된 시나리오 목록 | 배정할 시나리오 세 개 | 성향 응답에서 취약점 유형을 분류하는 일은 학습으로 다루는 편이 안정적이다. 규정된 시나리오 목록 밖으로는 나가지 않으며, 같은 응답에는 같은 시나리오가 배정된다. |
...
도식 5-2. LLM과 중앙 엔진의 경계

![시나리오 제작 LLM과 선택 ML 모델, 중앙 엔진, 평가 LLM, 규칙 코드의 역할 경계](figures/fig-2-system-boundary.png)

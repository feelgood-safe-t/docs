...
청산노노 MVP는 "판단 모드" 한 가지를 중심으로 구현했다. 사용자가 투자 성향 설문에 답하면 AI가 응답을 분석해 성향을 정리한 뒤, 검증된 카탈로그에서 시장 시나리오 세 개를 선별한다. 이후 가속 재생되는 시장 흐름 속에서 사용자가 근거와 확신도를 바탕으로 상승 또는 하락을 예측하면, 그 판단 과정을 일곱 가지 항목으로 나누어 채점한다. 각 항목별 점수와 평가 근거, 개선 방향을 함께 제공하며, 세 시나리오의 평균 점수가 70점 이상일 경우 "INVEST PASS"를 발급한다.
...
| AI 시나리오 배정 | 학습된 분류 모델이 검수된 카탈로그에서 사용자에게 맞는 시나리오 세 개 선택 |
...
| F-04 | AI 시나리오 배정 | 설문 응답을 입력으로 학습된 분류 모델이 검수된 카탈로그에서 사용자에게 맞는 시나리오 세 개를 고른다. | (서버) |
...
| F-18 | 결과 보고서 | 시나리오별 항목 점수와 평가 근거, 요약, 개선 방향을 제공한다. | 결과 |
...
| 최소 응답 | 각 시나리오에서 최소 한 번은 판단해야 완료할 수 있다. 판단 없이 끝난 시나리오는 0점으로 처리된다. |
...
사용자 상태는 브라우저에만 유지되며 서버에는 별도의 데이터베이스를 두지 않는다. 백엔드는 정적 데이터를 읽기 전용으로 참조하고, 입력값을 검증하여 스냅샷으로 고정한 뒤 AI로 전달하며, 점수 합산과 통과 여부 판정은 규칙 기반 코드가 처리한다.
...
| 시나리오 선택 (랜덤 포레스트) | 설문 응답, 검수된 시나리오 카탈로그 | 배정할 시나리오 세 개 |
| 과정 평가 | 봉인된 평가 입력 | 7개 항목별 점수와 근거 |

시나리오 콘텐츠는 실시간으로 만들지 않는다. 사전에 일괄 생성해 사람이 검수하고 승인한 것만 시나리오로 선택된다. 시나리오 선택은 설문 응답으로 학습한 랜덤 포레스트 분류 모델이 맡고, 성향 분석과 과정 평가는 생성형 모델이 맡는다. 세 가지 모두 검수된 카탈로그와 고정된 평가 기준 안에서만 이루어진다.
...
각 항목의 점수는 AI가 산출하되, 점수의 유효 범위와 합계는 규칙 코드가 직접 검증한다. 만약 점수 출력이 누락되거나 허용 범위를 벗어날 경우, 임의로 값을 보정하거나 대체하지 않고 즉각 실패로 처리한다.

평가 결과에는 입력값의 해시와 함께 평가 기준, 프롬프트, 모델, 결과 규칙의 버전을 함께 기록하여 사후에도 채점 기준을 명확히 추적할 수 있도록 한다.

동일한 정적 데이터와 내용상 같은 제출값은 언제나 동일한 스냅샷과 입력 해시를 생성한다. 다만 AI가 수행하는 성향 분석과 시나리오 선정, 항목별 점수 산출은 같은 입력값이라도 결과가 달라질 수 있으며, 최종 점수 합산과 PASS 여부 판정만이 규칙 코드를 통해 결정론적으로 이루어진다.
...
| 성향 응답 | 10문항 선택지 응답 | 성향 분석, 시나리오 선택 | 성향 요약, 배정된 시나리오 |
...
4단계의 평가 안내와 11단계의 결과 화면에서 확인할 수 있는 배점은 다음과 같다. 시나리오마다 100점 만점으로 채점하고 세 시나리오의 평균이 70점 이상이면 통과한다.

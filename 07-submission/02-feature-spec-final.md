...
| AI 시나리오 배정 | 검수된 카탈로그에서 시나리오 세 개를 고르고 배정 근거 제시 |
...
| F-04 | AI 시나리오 배정 | 검수된 카탈로그에서 사용자에게 맞는 시나리오 세 개를 고르고, 시나리오마다 배정 근거를 함께 보여준다 | 시나리오 배정 |
| F-05 | 시나리오 요약 안내 | 평가를 시작하기 직전에 배정된 세 시장 상황을 다시 요약해 보여준다 | 평가 준비 |
...
| F-18 | 과정 평가 | 제출된 판단 전체를 7개 항목으로 채점하고 항목마다 점수와 평가 근거를 생성한다 | (서버) |
...
한 화면에서 F-05부터 F-16까지가 함께 동작한다. 상단에 현재 문항과 남은 시간, 문항 완료와 시험 종료가 있고, 가운데에 시나리오 설명과 60배속으로 재생되는 캔들이, 오른쪽에 확신도와 판단 근거 입력이 놓인다.
...
| 4 | 시나리오 배정 | 배정 결과 확인 | 시나리오 세 개와 각각의 배정 근거가 표시된다 |
...
| 4 | 시나리오 배정 확인 | 시나리오 세 개와 각각의 배정 근거가 표시된다 |
